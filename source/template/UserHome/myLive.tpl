<!-- 公共头部 -->
<link rel="import" href="../Layer/layer.UserHome.head.tpl?__inline">
<!-- 学习中心区域 -->
<div class="container top m20">
    <div class="row">
        <div class="col-md-10 wrap-body">
            <!-- *********************** 内容区域开始 *********************** -->

            <link rel="import" href="../../widget/UserHome.live/live-index.tpl?__inline">
            <!-- 讲座内容 -->
            <div class="row row-page-turn container-live">
               <div class="live-begin live-hidden">
                   <div class="live-list">
                    <link rel="import" href="../../widget/UserHome.live/live-card.tpl?__inline">
                    <link rel="import" href="../../widget/UserHome.live/live-card.tpl?__inline">
                    <link rel="import" href="../../widget/UserHome.live/live-card.tpl?__inline">
                    <link rel="import" href="../../widget/UserHome.live/live-card.tpl?__inline">
                       <link rel="import" href="../../widget/UserHome.live/live-card.tpl?__inline">
                       <link rel="import" href="../../widget/UserHome.live/live-card.tpl?__inline">
                    </div>
                </div>
            </div>
            <!-- *********************** 内容区域结束 *********************** -->
        </div>
        <div class="col-md-2 col-sm-2 wrap-side">
            <link rel="import" href="../../widget/UserHome.sidebar/index.tpl?__inline">
        </div>
    </div>
</div>

<!-- 页面配置 -->
<script>
    var PAGE_CONFIG = {
        ID: 'Gold',
        MODULE: 'UserHome',
        TITLE: '金币明细-金币商城'
    };
</script>




<!-- 公共底部 -->
<link rel="import" href="../Layer/layer.UserHome.foot.tpl?__inline">
